...
def parse_calibration_file(calibration_file):
    ...
    try:
        ...
        # 查找"Extrinsic:"行，然后取下一行的矩阵数据
        extrinsic_found = False
        for i, line in enumerate(lines):
            if line.strip() == "Extrinsic:" and i > 10:  # 确保是json格式部分的Extrinsic
                if i + 1 < len(lines):
                    matrix_line = lines[i + 1].strip()
                    # 解析矩阵字符串
                    matrix_rows = matrix_line.split('],[')
                    
                    # 清理和解析每一行
                    matrix = []
                    for j, row_str in enumerate(matrix_rows):
                        # 清理字符串
        ...
        print("错误：未找到Extrinsic下的矩阵数据")
        return None
        
    except Exception as e:
        print(f"解析calibration文件失败: {e}")
        return None
...
def main():
    ...
    # 自动找到序号最大的calibration_*.txt文件
    calibration_files = glob.glob(os.path.join(current_dir, "calibration_*.txt"))
    if not calibration_files:
        print("错误: 未找到任何calibration_*.txt文件")
        return
    ...
    for file_path in calibration_files:
        ...
        # 提取calibration_和.txt之间的数字
        num_str = filename.replace("calibration_", "").replace(".txt", "")
        try:
            num = int(num_str)
            if num > max_num:
                max_num = num
    ...
    if calibration_file is None:
        print("错误: 未找到有效的calibration_*.txt文件")
        return
    data_dir = os.path.join(current_dir, "data")
    data_dir2 = os.path.join(current_dir, "..", "auto_calib", "data")

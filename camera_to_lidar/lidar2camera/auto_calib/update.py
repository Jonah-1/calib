...
def parse_calibration_json(calibration_file):
    """
    解析calibration_results.json文件中的外参矩阵
    返回: 4x4的外参矩阵列表
    """
    try:
        ...
        # 从JSON中提取外参矩阵
        if 'lidar_to_camera_extrinsics' in data and 'extrinsic_matrix' in data['lidar_to_camera_extrinsics']:
            matrix = data['lidar_to_camera_extrinsics']['extrinsic_matrix']
            
            # 验证矩阵格式
            if len(matrix) == 4 and all(len(row) == 4 for row in matrix):
                print("从JSON文件成功读取外参矩阵")
                return matrix
            else:
                print("错误：外参矩阵格式不正确")
                return None
        else:
            print("错误：JSON文件中未找到外参矩阵")
            return None
...
def main():
    ...
    calibration_file = os.path.join(current_dir, "calibration_results.json")
    ...
    # 1. 解析calibration_results.json中的外参矩阵
    print("\n1. 解析外参矩阵...")
    extrinsic_matrix = parse_calibration_json(calibration_file)

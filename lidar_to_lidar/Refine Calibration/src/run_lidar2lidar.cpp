...
void SaveCalibrationResults(const std::string &filename, const std::map<int32_t, Eigen::Matrix4d> &extrinsics) {
    std::ofstream file(filename);
    if (!file.is_open()) {
        std::cout << "[ERROR] Cannot open file " << filename << " for writing." << std::endl;
        return;
    }
    for (const auto &pair : extrinsics) {
        int32_t device_id = pair.first;
        const Eigen::Matrix4d &matrix = pair.second;
        
        file << "Device ID: " << device_id << std::endl;
...
int main(int argc, char *argv[]) {
  ...
  std::string output_filename = "calibration_results.txt"; // 你可以在这里指定文件路径
  SaveCalibrationResults(output_filename, refined_extrinsics);
